...
VALUES 					(	1	,		5		),
      ...
      -- bai tap 2 : lay tat ca id phong ban
      SELECT 		* FROM 		Department;



-- bai tap 3 : lấy ra id của phòng ban "Sale"
      SELECT 		DepartmentID FROM 		Department 
      WHERE 		DepartmentName = N'Sale';


-- bai tap 4 :  lấy ra thông tin account có full name dài nhất
SELECT 		* 
FROM 		`Account` 
...
WHERE length(FullName) = (SELECT Max_Fullname FROM cte_MaxLengthFullname);
      ...
      -- bai tap 5: Lấy ra thông tin account có full name dài nhất và thuộc phòng ban có id = 3
SELECT 		* 
FROM 		`Account` 
WHERE 		LENGTH(Fullname) = (SELECT MAX(LENGTH(Fullname)) FROM `Account`) ;
...
SELECT * FROM (SELECT * FROM `account` WHERE DepartmentID = 3) AS departmentId3
WHERE length(FullName) = (SELECT max(length(FullName)) FROM (SELECT * FROM `account` WHERE DepartmentID = 3) AS departmentId3);

-- bai tap 6 Lấy ra tên group đã tạo trước ngày 20/12/2019
SELECT 		GroupName  FROM 		`Group` 
WHERE 		CreateDate < '2019-12-20';



-- bai tap 7  Lấy ra ID của question có >= 4 câu trả lời
...
-- bai tap 8 Lấy ra các mã đề thi có thời gian thi >= 60 phút và được tạo trước ngày 20/12/2019
SELECT duration from exam	
where duration >= 60 and createdate < '2019-12-20' ;


-- bai tap 9 lay ra 5 group tao gan day nhat 
SELECT 		* FROM 		`Group`
ORDER BY 	CreateDate DESC 
LIMIT 5;

-- bai tap 10 Đếm số nhân viên thuộc department có id = 2
